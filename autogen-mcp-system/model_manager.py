...
class ModelManager:
    ...
    def get_model_client(self) -> OllamaChatCompletionClient:
        ...
        if self.rate_limit_until and time.time() > self.rate_limit_until:
            logger.info("⏰ Rate limit cooldown expired, switching back to primary model")
            self.fallback_active = False
            self.rate_limit_until = None
            self.failure_count = 0
    ...
    def handle_rate_limit(self, cooldown_minutes: int = 60):
        ...
        if not self.fallback_active:
            ...
            logger.info(f"💡 Note: Fallback model is local and may be slower but has no rate limits")
